...
{   clearInterval(intervalId);
    ...
    givenDate=document.querySelector("#dob").value;
        ...
        datenow=new Date();
         value=datenow-date1;
        if(value<0){assigning("#no-input","Sorry!" ,"Please enter correct date of birth");resetPartial();return null;}
        document.querySelector("#no-input").innerHTML=``;
        update();
        
    function assigning(x,y,z)
    {  
        document.querySelector(x).innerHTML=`<p style="padding:20px 20px 0 20px;">${y}</p>
        <p style="padding:0px 20px 20px 20px;">${z}</p>`;
     }    
    function update()
        { 
            date1=new Date(givenDate);
